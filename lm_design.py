...
class Designer:
    ...
    def calc_dist_loss(self, x_seq, objects, selection, reduction):
        ...
        bin_weight = torch.tensor([i for i in range(n_bin)]).to(res_preds['p_dist'])
        ...
        for ob in objects:
            ...
            for i in range(*ob[0]):
                for j in range(*ob[1]):
                    dist_ob.append(res_preds['p_dist'][0, i, j].dot(bin_weight))
            dist_ob = torch.tensor(dist_ob)
            dist.append(dist_ob.min() if selection == 'min' else dist_ob.mean())

        if reduction == 'sum':
            return torch.tensor(dist).sum()
        elif reduction == 'prod':
